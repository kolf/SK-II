$(document)
  .ready(function () {
    ...
    PRODUCTS
      ...
      .forEach(function (item, index) {
        if (index < 3) {
          ...
          top3Html.push('<div class="col-4 mb-4"><div onclick="Tracker(' + item.id + ')" class="card pt-3"><div class="card-body"><h4 class="card-title">' + item.title + '</h4><p class="card-text">' + item.subTitle + '</p></div><img class="card-img-top" src="' + item.imgUrl + '"></div></div>')
        }

        sectionHtml.push('<div class="page-section p-3 p-lg-5 h-100 mx-auto flex-column js-page-block" id=' +
          '"section' + APP_KEY + item.id + '" style="background-image: url([%GraphicsPath()%]/img/FTE_image.jpg)"><div class' +
          '="my-auto"><div class="row"><div class="col-7 offset-5"><div class="bg-white p-l' +
          'g-5 p-5 h-100"><div class="text-center mb-5"><h2 class="text-warning">' + item.title + "</h2><h3>" + item.subTitle + "</h3></div>" + item.content + "</div></div></div></div></div>");
        ...
        productsHtml.push('<div class="col" id="item_' + item.id + '"><div class="card mb-4 box-shadow"><img class="card-img-top" src="' + item.imgUrl + '"><div class="card-body"><h5 class="card-title product-title">' + item.title + '</h5><p class="card-text">' + item.size + '</p><div class="btns"><button type="button" class="btn btn-sm btn-secondary js-b' +
          'uy-btn" data-id="' + item.id + '">BUY</button><button type="button" onclick="Tracker(' + item.id + ')"  class="btn btn-sm btn-secondary js-scroll-trigger" data-href="#' + APP_KEY + item.id + '">DETAIL</button></div></div></div></div>');
    ...
    sectionHtml.push('<div class="page-section p-3 p-lg-5 h-100 mx-auto flex-column js-page-block" id=' +
      ...
      '><span id="cart-num">0</span></div><button class="btn btn-primary js-scroll-trig' +
      'ger" href="#myCart">MY CART</button></div></div></div>');

    sectionHtml.unshift('<div class="page-section h-100 mx-auto p-3 p-lg-5 flex-column js-page-block" id=' +
      '"section' + APP_KEY + PRODUCTS.id + '"><div class="my-auto row text-center"><div class="col-sm-8"><div class="row">' + top3Html.join("") + '</div></div><div class="col-sm-4"><div class="my-auto"><h3 class="p-3">' + PRODUCTS.title + "</h3>" + PRODUCTS.content + "</div></div></div></div>");
    ...
    var bodyHtml = '<nav class="navbar navbar-expand-lg _bg-white navbar-dark bg-dark _bg-primary fi' +
      ...
      'colspan="5">您的购物车还没有产品...</td></tr></tbody></table></div><div class="text-right"' +
      '><span class="display-4" id="totalPrice">$0</span><div><button class="btn btn-pr' +
      'imary" id="next_button">CHECKOUT</button></div></div></div></div>' + hiddenInputs.join('') + '<script src="../../graphics/vendor/bootstrap/js/bootstrap.bundle.min.js"></scrip' +
